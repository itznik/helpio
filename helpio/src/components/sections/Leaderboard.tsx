    ...
    <section className="py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-amber-500/20 bg-amber-500/10 text-amber-600 dark:text-amber-300 text-xs font-bold uppercase tracking-widest mb-4">
        ...
        <div className="flex flex-col md:flex-row justify-center items-end gap-6 mb-16 px-4">
          <PodiumCard donor={DONORS[1]} delay={0.2} height="h-64" color="border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-800" icon={<Medal className="w-6 h-6 text-slate-500 dark:text-slate-300" />} />
          <PodiumCard donor={DONORS[0]} delay={0} height="h-80" color="border-amber-400 bg-amber-50 dark:bg-amber-900/20" icon={<Trophy className="w-8 h-8 text-amber-500" />} isFirst />
          <PodiumCard donor={DONORS[2]} delay={0.4} height="h-56" color="border-orange-300 dark:border-orange-800 bg-orange-50 dark:bg-orange-900/10" icon={<Medal className="w-6 h-6 text-orange-600" />} />
        </div>
          ...
          {DONORS.slice(3).map((donor, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 dark:text-white group-hover:text-amber-500 transition-colors">{donor.name}</h4>
...
function PodiumCard({ donor, delay, height, color, icon, isFirst = false }: any) {
  return (
    <motion.div
      ...
      className={`relative flex flex-col items-center justify-end w-full md:w-44 ${height} rounded-t-3xl border-t border-x ${color} backdrop-blur-sm pb-6`}
    >
      <div className={`absolute -top-10 left-1/2 -translate-x-1/2 w-20 h-20 rounded-full border-4 ${isFirst ? 'border-amber-400' : 'border-slate-200 dark:border-slate-700'} overflow-hidden bg-white dark:bg-slate-900`}>
        <img src={donor.avatar} alt={donor.name} className="w-full h-full object-cover" />
      </div>
      <div className="mb-3">{icon}</div>
      <h3 className={`font-bold text-lg ${isFirst ? 'text-amber-600 dark:text-amber-300' : 'text-slate-900 dark:text-white'}`}>{donor.name}</h3>
      <p className="text-slate-500 text-sm mb-2">{donor.badge}</p>
      <div className={`px-4 py-1 rounded-full text-sm font-bold ${isFirst ? 'bg-amber-500 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-white'}`}>
