...
export default function WishCard({ name, avatar, wish, story, amount, category, verified }: WishCardProps) {
  return (
    <motion.div 
      ...
      className="h-full"
    >
      <div className="glass-panel rounded-3xl p-6 h-full flex flex-col relative z-10 transition-all hover:border-teal-500/30 group">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-white/5 mt-auto">
          ...
          <div className="flex gap-2">
             ...
             {/* FIXED BUTTON: Always dark bg with white text in light mode */}
             <button className="px-5 py-2 rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-bold shadow-lg hover:opacity-90 transition-opacity">
               Grant
             </button>
          </div>
        </div>
